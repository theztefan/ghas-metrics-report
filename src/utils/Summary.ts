...
export function prepareSummary(report: Report): void {
  ...
  const dependabotTop10rows: SummaryTableRow = report.dependabot_metrics?.top10.map((a: any) => [
  ...
  const codeScanningTop10rows: SummaryTableRow = report.code_scanning_metrics?.top10.map(
    (a: any) => [
      a.rule?.name,
      a.rule?.severity,
      a.tool?.name,
  ...
  const secretScanningTop10rows: SummaryTableRow = report.secret_scanning_metrics?.top10.map(
    (a: any) => [
